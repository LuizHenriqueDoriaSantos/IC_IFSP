...
def recordDistancesDirections(i, j, origins, destinations, tempmode, distance, paths, city, state, duration):
    ...
    sql = "INSERT INTO " + tableName + " (idx_origin, idx_destination, origin, destination, distance_meters, mode, path, duration) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)"

    # Check if query have benn performed successfully
    print(sql,str(i), str(j), str(origins), str(destinations), str(distance), str(tempmode), str(path_str), str(duration))
    result = cursor.execute(sql, (str(i), str(j), str(origins), str(destinations), str(distance), str(tempmode), str(path_str), str(duration)))
    ...
    if result == None:
        ...
        if distance != -1:
            ...
            while w < numDirections:
                ...
                tt = 'id_' + tableName
                sql = "INSERT INTO " + intermediateTableName + " (" + tt + ", duration_seconds, start_point, end_point, mode, path, distance_meters) VALUES (%s, %s, %s, %s, %s, %s, %s)"
                cursor.execute(sql, (str(id_inserted_row), str(duration), str(start_location), str(end_location), str(travel_mode), str(encoded_points), str(distance)))
                con.commit()
